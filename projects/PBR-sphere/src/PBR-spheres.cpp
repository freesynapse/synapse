...
public:
	...
	Ref<Shader> m_basicLightShader = nullptr;
	Ref<Shader> m_PBRShader = nullptr;
	Ref<Mesh> m_sphere = nullptr;

	bool m_selectedPBR = true;
...
class AppInst : public Application
{
public:
	AppInst() { this->pushLayer(new layer); }
};
Application* CreateSynapseApplication() { return new AppInst(); }
...
void layer::onAttach()
{
	...
	m_camera->setPosition({ 0.0f, 0.0f, 18.0f });
	...
	ShaderLibrary::load("../assets/shaders/PBR/lightShader.glsl");
	ShaderLibrary::load("../assets/shaders/PBR/PBRShader.glsl");
	m_basicLightShader = ShaderLibrary::get("lightShader");
	m_PBRShader = ShaderLibrary::get("PBRShader");
...
void layer::onUpdate(float _dt)
{
	...
	{
		m_PBRShader->enable();
		m_PBRShader->setMatrix4fv("u_view_projection_matrix", m_camera->getViewProjectionMatrix());
		m_PBRShader->setUniform3fv("u_camera_pos", m_camera->getPosition());
		// material
		m_PBRShader->setUniform3fv("u_albedo", m_materialAlbedo);
		m_PBRShader->setUniform1f("u_metallic", m_materialMetallic);
		m_PBRShader->setUniform1f("u_roughness", m_materialRoughness);
		m_PBRShader->setUniform1f("u_ao", 1.0f);
		// render
		m_sphere->render(m_PBRShader);
	}
	else
	{
		m_basicLightShader->enable();
		m_basicLightShader->setMatrix4fv("u_view_projection_matrix", m_camera->getViewProjectionMatrix());
		glm::mat3 normal_matrix = glm::transpose(glm::inverse(m_sphere->getTransform().getModelMatrix()));
		m_basicLightShader->setMatrix3fv("u_normal_matrix", normal_matrix);
		m_basicLightShader->setUniform3fv("u_camera_pos", m_camera->getPosition());
		// material
		m_basicLightShader->setUniform3fv("u_albedo", glm::vec3(0.7f, 0.2f, 0.2f));
		// render
		m_sphere->render(m_basicLightShader);
	}
...
void layer::onImGuiRender()
{
	...
	//-----------------------------------------------------------------------------------
	ImGui::Begin("synapse-core::material-selection");
