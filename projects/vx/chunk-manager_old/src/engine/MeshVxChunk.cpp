...
namespace Syn 
{
	...
	void MeshVxChunk::generateVoxels()
	{
		...
		/* Default, naive voxel generation... */
		{
			// Create the voxel data. Normalized fractal perlin noise in 5 octaves.
			//FastNoise noise;
			//noise.SetFractalOctaves(5);
			//noise.SetFrequency(0.01f);

			// Later replaced by some sort of BiomeGenerator class call
			// m_parentManager->m_biomeGenerator->setNoiseParameters(chunk.x, chunk.z)
			// for now static
			Noise::set_param_base_amp(10.0f);
			...
			VX_FOR_BEGIN_DEF
				...
				//float n = noise.GetPerlinFractal2(v.x, v.y);
				float n = Noise::fbm_perlin2(v);
				// normalize and scale
				n = (n + 1.0f) * 0.5f * m_meshScale;
				// translate to y-value
				unsigned char h = static_cast<unsigned char>(n * 255.0f);
