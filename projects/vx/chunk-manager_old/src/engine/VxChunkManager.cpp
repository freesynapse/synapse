...
namespace Syn 
{
	...
	//-----------------------------------------------------------------------------------
	void VxChunkManager::debugUpdateChunksFromTexture(const Ref<Texture2DNoise>& _noise_texture_ptr)
	{
		int chunkCount;
		static bool first = true;
		if (first)
		{
			chunkCount = _noise_texture_ptr->getWidth() / VX_CHUNK_SIZE_XZ;
		...
		// replace loadPendingChunks() with inline since a different
		// MeshVxChunk constructor is called.
		if (m_pendingChunksLoad.size() > 0)
		{
			...
			for (uint32_t i = 0; i < m_chunkLoadCount; i++)
			{
				...
				m_chunkMap[chunkPos] = new MeshVxChunk(this, chunkPos, _noise_texture_ptr);
	...
	//-----------------------------------------------------------------------------------
	void VxChunkManager::debugUpdateVoxelDataFromTexture(const Ref<Texture2DNoise>& _noise_texture_ptr, float _mesh_scale)
	{
		for (auto& it : m_chunkMap)
		{
			it.second->setMeshScale(_mesh_scale);
			it.second->generateVoxelsFromTexture(_noise_texture_ptr);
			m_pendingChunksMesh.push_back(it.first);
		}
	...
	float VxChunkManager::debugDestroyVoxels(const glm::ivec3& _min, const glm::ivec3& _max)
	{
		...
		SYN_CORE_TRACE("n = ", m_remeshSet.size());
	...
	void VxChunkManager::debugSetupChunkBoundaries()
	{
		...
		// Warning	C26444	Avoid unnamed objects with custom constructionand destruction.
