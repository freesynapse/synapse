...
class graph_visualization : public Syn::Application
{
public:
	graph_visualization() { this->pushLayer(new layer); }
};
Syn::Application* CreateSynapseApplication() { return new graph_visualization(); }
...
void layer::onAttach()
{
	...
	m_graph = Syn::MakeRef<Syn::Graph>(m_fboRender);
	m_graph->loadGraphFromFile("/home/iomanip/documents/_BIOINFO--ML/_EXP/graph-theory/graphs/BA-1000-KK.graph");
	//m_graph->loadGraphFromFile("/home/iomanip/documents/_BIOINFO--ML/_EXP/graph-theory/graphs/BA-5000-KK.graph");
...
void layer::onUpdate(float _dt)
{
	...
	int i = 0;
	m_font->addString(2.0f, fontHeight * ++i, "fps=%.0f  VSYNC=%s", Syn::TimeStep::getFPS(), Syn::Application::get().getWindow().isVSYNCenabled() ? "ON" : "OFF");
	glm::vec3 camPos = m_camera->getPosition();
	m_font->addString(2.0f, fontHeight * ++i, 
					  "camera [ %.1f  %.1f  %.1f ]  theta_x=%.1f  theta_y=%.1f", 
					  camPos.x, camPos.y, camPos.z,
					  m_camera->getXAngle(), m_camera->getYAngle());
	m_font->addString(2.0f, fontHeight * ++i, "nodes: %d", m_graph->getNodeCount());
	m_font->addString(2.0f, fontHeight * ++i, "edges: %d", m_graph->getEdgeCount());
...
void layer::onKeyDownEvent(Syn::Event* _e)
{
	...
	if (e->getAction() == GLFW_PRESS)
	{
		...
		{
		case SYN_KEY_R:
			Syn::ShaderLibrary::reload("graphPointShader2");
			break;
