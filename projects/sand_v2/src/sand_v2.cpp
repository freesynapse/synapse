...
public:
	...
	Ref<SandWorld> m_world = nullptr;
	Ref<Shader> m_sandShader = nullptr;
...
class sand_v2 : public Application
{
public:
	sand_v2() { this->pushLayer(new layer); }
};
Application* CreateSynapseApplication() { return new sand_v2(); }
...
void layer::onAttach()
{
	...
	ShaderLibrary::load("../assets/shaders/Synapse-sand/sandShader_v2.glsl");
	m_sandShader = ShaderLibrary::get("sandShader_v2");
	m_world = MakeRef<SandWorld>(0, 0, 4, m_sandShader);
...
void layer::onKeyDownEvent(Event* _e)
{
	...
	if (e->getAction() == GLFW_PRESS)
	{
		...
		case SYN_KEY_X:
			m_world->reset();// = new SandWorld(0, 0, 2, m_sandShader);
			break;
