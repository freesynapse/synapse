...
{	
	...
	char Log::s_debug_buffer[1024];
	char Log::s_debug_func_buffer[512];
	...
	void Log::debug_matrix(const char* _func, const char* _mat_name, const glm::mat4& _m4)
	{
		...
		memset(s_debug_buffer, 0, 1024);
		int c = 0;
		c += std::sprintf(s_debug_buffer, "%s\n", _mat_name);
		c += std::sprintf(s_debug_buffer + c, "|  %s%.2f  %s%.2f  %s%.2f  %s%.2f  |\n", ff(_m4[0][0]), fabs(_m4[0][0]), ff(_m4[0][1]), fabs(_m4[0][1]), ff(_m4[0][2]), fabs(_m4[0][2]), ff(_m4[0][3]), fabs(_m4[0][3]));
		...
		std::sprintf(s_debug_buffer + c, "|  %s%.2f  %s%.2f  %s%.2f  %s%.2f  |", ff(_m4[3][0]), fabs(_m4[3][0]), ff(_m4[3][1]), fabs(_m4[3][1]), ff(_m4[3][2]), fabs(_m4[3][2]), ff(_m4[3][3]), fabs(_m4[3][3]));

		Log::log_no_func(s_debug_func_buffer, s_debug_buffer);
	...
	void Log::debug_matrix(const char* _func, const char* _mat_name, const glm::mat3& _m3)
	{
		...
		c += std::sprintf(s_debug_buffer, "%s\n", _mat_name);
		c += std::sprintf(s_debug_buffer + c, "|  %s%.2f  %s%.2f  %s%.2f  |\n", ff(_m3[0][0]), fabs(_m3[0][0]), ff(_m3[0][1]), fabs(_m3[0][1]), ff(_m3[0][2]), fabs(_m3[0][2]));
		c += std::sprintf(s_debug_buffer + c, "|  %s%.2f  %s%.2f  %s%.2f  |\n", ff(_m3[1][0]), fabs(_m3[1][0]), ff(_m3[1][1]), fabs(_m3[1][1]), ff(_m3[1][2]), fabs(_m3[1][2]));
		std::sprintf(s_debug_buffer + c, "|  %s%.2f  %s%.2f  %s%.2f  |", ff(_m3[2][0]), fabs(_m3[2][0]), ff(_m3[2][1]), fabs(_m3[2][1]), ff(_m3[2][2]), fabs(_m3[2][2]));
		Log::log_no_func(s_debug_func_buffer, s_debug_buffer);
	...
	void Log::debug_vector(const char* _func, const char* _vec_name, const glm::vec4& _v4)
	{
		...
		c += std::sprintf(s_debug_buffer, "%s  ", _vec_name);
		std::sprintf(s_debug_buffer + c, "|  %s%.2f  %s%.2f  %s%.2f  %s%.2f  |", ff(_v4.x), fabs(_v4.x), ff(_v4.y), fabs(_v4.y), ff(_v4.z), fabs(_v4.z), ff(_v4.w), fabs(_v4.w));
		Log::log_no_func(s_debug_func_buffer, s_debug_buffer);
	...
	void Log::debug_vector(const char* _func, const char* _vec_name, const glm::vec3& _v3)
	{
		...
		c += std::sprintf(s_debug_buffer, "%s  ", _vec_name);
		std::sprintf(s_debug_buffer + c, "|  %s%.2f  %s%.2f  %s%.2f  |", ff(_v3.x), fabs(_v3.x), ff(_v3.y), fabs(_v3.y), ff(_v3.z), fabs(_v3.z));
		Log::log_no_func(s_debug_func_buffer, s_debug_buffer);
	...
	void Log::debug_vector(const char* _func, const char* _vec_name, const glm::vec2& _v2)
	{
		...
		c += std::sprintf(s_debug_buffer, "%s  ", _vec_name);
		std::sprintf(s_debug_buffer + c, "|  %s%.2f  %s%.2f  |", ff(_v2.x), fabs(_v2.x), ff(_v2.y), fabs(_v2.y));
		Log::log_no_func(s_debug_func_buffer, s_debug_buffer);
	...
	void Log::debug_vector(const char* _func, const char* _vec_name, const glm::ivec4& _iv4)
	{
		...
		c += std::sprintf(s_debug_buffer, "%s  ", _vec_name);
		std::sprintf(s_debug_buffer + c, "|  %s%d  %s%d  %s%d  %s%d  |", fi(_iv4.x), abs(_iv4.x), fi(_iv4.y), abs(_iv4.y), fi(_iv4.z), abs(_iv4.z), fi(_iv4.w), abs(_iv4.w));
		Log::log_no_func(s_debug_func_buffer, s_debug_buffer);
	...
	void Log::debug_vector(const char* _func, const char* _vec_name, const glm::ivec3& _iv3)
	{
		...
		c += std::sprintf(s_debug_buffer, "%s  ", _vec_name);
		std::sprintf(s_debug_buffer + c, "|  %s%d  %s%d  %s%d  |", fi(_iv3.x), abs(_iv3.x), fi(_iv3.y), abs(_iv3.y), fi(_iv3.z), abs(_iv3.z));
		Log::log_no_func(s_debug_func_buffer, s_debug_buffer);
	...
	void Log::debug_vector(const char* _func, const char* _vec_name, const glm::ivec2& _iv2)
	{
		...
		c += std::sprintf(s_debug_buffer, "%s  ", _vec_name);
		std::sprintf(s_debug_buffer + c, "|  %s%d  %s%d  |", fi(_iv2.x), abs(_iv2.x), fi(_iv2.y), abs(_iv2.y));
		Log::log_no_func(s_debug_func_buffer, s_debug_buffer);
	}
	...
	void Log::debug_AABB(const char* _func, const char* _aabb_name, const AABB& _aabb)
	{
		...
		c += std::sprintf(s_debug_buffer, "%s: %s  ", _func, _aabb_name);
		std::sprintf(s_debug_buffer + c, "min |  %s%d  %s%d  %s%d  |   max |  %s%d  %s%d  %s%d  |", 
