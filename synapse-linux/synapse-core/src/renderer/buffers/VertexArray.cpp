...
namespace Syn {	
	...
	//-----------------------------------------------------------------------------------
	VertexArray::VertexArray(Ref<VertexBuffer> _vertex_buffer_ref, Ref<IndexBuffer> _index_buffer_ref)
	{
		if (!m_arrayID)
		{
			SYN_RENDER_S0({
	...
	//-----------------------------------------------------------------------------------
	VertexArray::VertexArray(Ref<VertexBuffer> _vertex_buffer_ref)
	{
		if (!m_arrayID)
		{
			SYN_RENDER_S0({
	...
	//-----------------------------------------------------------------------------------
	void VertexArray::setVertexBuffer(Ref<VertexBuffer> _vertex_buffer)
	{
	...
	//-----------------------------------------------------------------------------------
	void VertexArray::setIndexBuffer(Ref<IndexBuffer> _index_buffer)
	{
	...
	//-----------------------------------------------------------------------------------
	void VertexArray::updateVertexBuffer(void* _vertices, uint32_t _size, uint32_t _offset)
	{
		m_vertexBuffer->updateBufferData(_vertices, _size, _offset);
	}
