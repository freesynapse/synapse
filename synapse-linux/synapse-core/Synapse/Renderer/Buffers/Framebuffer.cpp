...
namespace Syn {
	...
	void FramebufferBase::saveAsPNG(const std::string& _filename)
	{
		...
		std::string fileName = _filename == "" ? dirName + Time::current_date() + '_' + time + '_' + Random::rstr(24) + ".png" : _filename;
	...
	void FramebufferBase::init(const glm::ivec2& _size)
	{
		...
		SYN_RENDER_S0({
			...
			if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
			{
				...
				SYN_CORE_TRACE("Framebuffer '", self->m_name, "'; renderTargets=", self->m_colorAttachmentCount, " [ ", self->m_size.x, "x", self->m_size.y, " ] created.");
			#endif
